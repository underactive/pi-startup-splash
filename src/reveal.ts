 ...
 *
 * Falls back to the settled line whenever a frame could not be drawn faithfully: a 256-color theme
 * has no channels to interpolate, a `fitCell`-truncated tagline carries escapes that must not be
 * split per character, a surrogate pair would desynchronize the code point index from the
 * `visibleLength` the panel budgets by, and a tagline shorter than the placeholder could never
 * overwrite all of it, so the label would still be showing once the wipe ran out of characters.
 */
