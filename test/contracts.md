...
| S-07 | `buildHeader` collapse | when panel would exceed `MAX_SPLASH_ROW_SHARE` of termRows **or** the longest name would not fit → both lists collapse to one counts line; otherwise names appear inline | README; amended 2026-07-29 (F-16: the `state.skillsExtensionsListed` half was dropped with the flag — the branch is now asserted on rendered output) |
