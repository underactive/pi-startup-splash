...
| T-15 | `fitCell` | UNSPECIFIED: `width < ELLIPSIS.length` — assert only: no throw, `visibleLength <= max(width,0)`... if this fails it is a finding, not a spec | — |
...
| S-03 | `buildCountsLine` | contains `[skills] N` and `[extensions] M` with the actual counts, `·`-joined; `<= width` | doc comment (`[skills] 22 · [extensions] 33`) |
...
| S-07 | `buildHeader` collapse | when panel would exceed `MAX_SPLASH_ROW_SHARE` of termRows **or** the longest name would not fit → both lists collapse to one counts line; `state.skillsExtensionsListed` set false; otherwise names appear inline and flag true | README + `state.ts` doc comment on `skillsExtensionsListed` |
...
| # | Symbol | Claim | Source |
|---|--------|-------|--------|
| D-01 | `extensionNameFromPath` | resolution order: (1) `<inline:name>` → verbatim; (2) `node_modules/@scope/name/...` → `@scope/name`; (3) `node_modules/name/...` → `name`; (4) `…/pkg/src/index.ts` (holder dir in `ENTRY_HOLDER_DIRS`) → package name from pkg's package.json; (5) `…/ext-name/index.ts` → `ext-name`; (6) `…/ext-name.ts` → `ext-name` | doc comment (numbered list) |
| D-02 | `cliExtensionArgs` | parses this process's argv: `--no-extensions` flag; explicit `--extension`/`-e` values (space and `=` forms — `=` form per common CLI convention, mark `=`-form INFERRED) | doc comment |
| D-03 | `readNpmPackageName` | package.json `name` when present and a string; `undefined` when file missing or unparseable | doc comment |
| D-04 | `packageNameForEntryHolder` | for an entry dir in `ENTRY_HOLDER_DIRS`, resolve owning package's name; dirs that already name their extension are left alone (→ undefined) | doc comment |
...
| # | Symbol | Claim | Source |
|---|--------|-------|--------|
| X-01 | constants | `GATE_DONE_ENV === "PI_SPLASH_GATE_DONE"` | exported literals + README; amended 2026-07-28 (the `SKIP_GATE_FLAG` clause dropped with the `--skip-splash-gate` flag — `/startup-splash menuGate:on\|off` is the only toggle) |
...
| GA-07 | `skillsInline` | when true, the Skills and Extensions row is hidden (redundant with splash listing) and its hotkey inert | activeMenu doc comment |
...
| # | Symbol | Claim | Source |
|---|--------|-------|--------|
| I-01 | registration | default export registers **no** CLI flags; command `startup-splash`; handlers for `model_select`, `before_agent_start`, `session_start` | README (Commands, Troubleshooting) + explorer signature scan; amended 2026-07-28 (`--skip-splash-gate` removed — the slash command is the only toggle) |
...
| I-06 | `/startup-splash menuGate:off` | persists the gate-menu opt-out and notifies; no header/settings side effects (the gate is decided at startup); a failed preference write (either branch) notifies an error, never success | README Commands; amended 2026-07-28 (slop-audit F-11: failure path added) |
| I-07 | `/startup-splash menuGate:on` | persists the gate-menu opt-in and notifies; an unrecognized argument notifies an error and writes nothing | README Commands |
...
## Findings (report-don't-reconcile: F-2, F-3, F-6 are `it.todo` tests — failures reported, suite exit stays green)
...
| F-2 | `fitCell(text, width < 3)` | emits the full 3-col ellipsis, exceeding the requested width | real edge bug vs. the W invariant; unreachable from current callers but a hazard |
...
| F-6 | `renderFooter` bodyWidth < ~26 | Select/Cancel row is never truncated below its natural width | edge W violation on very narrow popups |
| F-7 | `discoverExtensionNamesFromFilesystem` | a dir without any entry file is still reported (`bare-dir`) | open question: over-reporting, or intentional optimism? (unasserted) |
...
| F-9 | `panelBg` doc | says "WCAG relative luminance"; code uses Rec.601 luma coefficients | identical for greys (tests unaffected); doc nit |
...
| F-11 | `/startup-splash menuGate:*` | `writeMenuGate` swallowed write failures while the command handler notified success unconditionally — a failed write was reported as persisted | resolved (2026-07-28 slop audit: `writeMenuGate` returns a boolean, mirroring `ensureQuietStartup`; failure now notifies an error, covered by an I-06/I-07 test) |
